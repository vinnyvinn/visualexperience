...
class Product_controller extends Home_Core_Controller
{
	...
	public function add_product_post()
	{
		...
		if ($this->product_model->add_product()) {
			...
			$this->tag_model->add_product_tags($last_id);
			//add product images
	...
	public function tag($slug)
	{
		...
		$data['tag'] = $this->tag_model->get_product_tag($slug);
		if (empty($data['tag'])) {
			redirect(lang_base_url() . "products");
		}
		...
		$link = lang_base_url() . 'product/tag'. $data['tag']->tag_slug;
		$pagination = $this->paginate($link, $this->product_model->get_paginated_tag_products_count($data['tag']->tag_slug), $this->product_per_page);
		$data['products'] = $this->product_model->get_paginated_tag_products(null, null, null,$pagination['per_page'], $pagination['offset'], $data['tag']->tag_slug);
		...
		$this->load->view('partials/_header', $data);
		$this->load->view('product/tag', $data);
		$this->load->view('partials/_footer');
	}
	/**
	 * Edit Draft
	...
	public function edit_draft($id)
	{
		...
		$tags = "";
		$count = 0;
		$tags_array = $this->tag_model->get_product_tags($id);
